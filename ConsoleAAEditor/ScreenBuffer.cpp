...
void ScreenBuffer::CursorPosition(int x, int y) {

    COORD CursorPos;   //カーソルの座標
    CursorPos.X = x;
    CursorPos.Y = y;


    SetConsoleCursorPosition(mHandle, CursorPos);
}



void ScreenBuffer::DrawAA(int H,int W, const char* format, ...)
{
    ...
    // 日本語じゃない時
    if (wc[0] >= 0x00 && wc[0] <= 0xFF)
    {
        mScreenBuffer[H][mNowBuffer_width[H] + W].Char.UnicodeChar = wc[0];
        mScreenBuffer[H][mNowBuffer_width[H] + W].Attributes = FOREGROUND_GREEN;
    }
...
void ScreenBuffer::Draw(int H ,int W,const char* format, ...)
{
    ...
    for (; mNowBuffer_width[H] <= MAX_WIDTH; mNowBuffer_width[H]++) {
        ...
        if (wc[i] != L'\0') {
            ...
            {
                mText->Write("横のの表示文字数を超えました描画できません。 \n");
                exit(1);
